...
// Mirror response from remote JVM to stdout or directByteBuffer
static int read_response(int fd, JNIEnv* env, jobject directByteBuffer) {
	...
	if (directByteBuffer != NULL) {
        ...
        if(remaining < 8){
			...
			if (remaining > 0) {
                ...
                int to = remaining < bytes ? remaining : bytes;
                for (int i = 0; i < to; i++) {
		...
		ssize_t bytes = read(fd, buf + position, remaining);
        ...
        do {
            ...
            bytes = read(fd, buf + position, remaining);
...
JNIEXPORT jint JNICALL Java_jattach_Jattach_exec(JNIEnv* env, jclass cl, jint jpid, jstring cmd, jstring option1, jstring option2, jstring option3, jobject directByteBuffer) {
    ...
    if ((my_gid != target_gid && setegid(target_gid) != 0) ||
        ...
        return FAILED_TO_CHANGE_CREDENTIALS;
    }
    ...
    if (!check_socket(nspid) && !start_attach_mechanism(pid, nspid)) {
        return COULD_NOT_START_ATTACH_MECHANISM;
    }
    ...
    int result = read_response(fd, env, directByteBuffer);
    if (directByteBuffer == NULL) {
        printf("\n");
    }
    close(fd);
